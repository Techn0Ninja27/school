...
def stringUpper(string):
    '''if 2 of first 4 characters are uppercase,
     return entire string uppercase'''
    ...
    if amountUppercase >= 2:  # checks if there are 2 or more uppercase chars
...
def caesarCipher(string, shift):
    ...
    for i in range(0, len(string)):  # loops for the length of the string
        ...
        # if character is uppercase
            ...
            # 69, subtracting 65 from 69 returns 4,
            #
            # which is the position of the character e
            # in the alphabet if counting from 0
            #
            # using the %(modulus) operator then makes the number not go over
            # 26
            #
            # eg 28 % 26 returns 2, and 5 % 26 returns 5
            # + 65 then makes the character go back to its original
            # unicode value
            #
...
def stringStrip(string, characters):
    ...
    for i in string:  # iters through string
        ...
        if inString is False:  # if character is not to be stripped, it will be
...
def countRepeat(string):
    ...
    for i in repeatChars:  # iters through repeated characters
        # prints repeated charcter and number of times it was repeated
        print(i, repeatChars[i])
